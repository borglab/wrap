...
classdef MyTemplatePoint2 < MyBase
  ...
  methods
    function obj = MyTemplatePoint2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = geometry_wrapper(50, varargin{2});
        end
        base_ptr = geometry_wrapper(49, my_ptr);
      elseif nargin == 0
        [ my_ptr, base_ptr ] = geometry_wrapper(51);
      else
    ...
    function delete(obj)
      geometry_wrapper(52, obj.ptr_MyTemplatePoint2);
    end
  ...
  methods(Static = true)
    function varargout = Level(varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'double') && size(varargin{1},1)==2 && size(varargin{1},2)==1
        varargout{1} = geometry_wrapper(64, varargin{:});
        return
      end
