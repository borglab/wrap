...
"""
    ...
    def test_unknown_callable_annotation(self):
        error = self.assert_parse_error(
            "@pybind_lambda int function();",
            line=1,
            column=1,
            context="callable annotation",
            expected="malformed or unknown annotation '@pybind_lambda'",
        )
        self.assertIn("@pybind_adapter", error.hint)

    def test_malformed_callable_annotation(self):
        self.assert_parse_error(
            "@pybind-adapter int function();",
            line=1,
            column=1,
            context="callable annotation",
            expected="malformed or unknown annotation '@pybind-adapter'",
        )

    def test_misplaced_callable_annotation(self):
        error = self.assert_parse_error(
            "class Foo { @pybind_adapter Foo(); };",
            line=1,
            column=13,
            context="callable annotation",
            expected=(
